...
const ROOT_ENV_EXAMPLE = path.join(PACKAGE_ROOT, "env.example");
...
function installSkills(options) {
  ...
  const sharedEnvTargetPath = path.join(destinationRoot, "env.example");
  if (fs.existsSync(ROOT_ENV_EXAMPLE) && (force || !fs.existsSync(sharedEnvTargetPath))) {
    fs.copyFileSync(ROOT_ENV_EXAMPLE, sharedEnvTargetPath);
  }
