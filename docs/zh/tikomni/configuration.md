...
3. `docs/zh/tikomni/runtime-config.md`：用户可见执行策略配置。
...
4. `docs/zh/tikomni/runtime-config.md`：策略配置（输出目录、标签、策略）。
...
`runtime-config.md` 是本 skill 的策略配置文档，建议包含以下区块：
...
1. 调用任何接口前，先读取 `runtime-config.md`（或用户指定配置路径）。
