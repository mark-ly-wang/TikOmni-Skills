...
def _normalize_http_summary(summary: Optional[Dict[str, Any]]) -> Dict[str, Any]:
...
class ProgressReporter:
    ...
    def http_event(
        ...
        *,
        ...
        payload = _normalize_http_summary(summary)
        payload.update(
            {
                "endpoint": str(endpoint or ""),
                "route_label": str(route_label or ""),
                "ok": bool(response_obj.get("ok")),
                "status_code": response_obj.get("status_code"),
                "request_id": response_obj.get("request_id"),
                "error_reason": response_obj.get("error_reason"),
            }
        )
        self.progress(stage=stage, message="http event", data=payload)

    def heartbeat(self, *, stage: str, message: str = "", data: Optional[Dict[str, Any]] = None) -> None:
        self.progress(stage=stage, message=message, data=data)
