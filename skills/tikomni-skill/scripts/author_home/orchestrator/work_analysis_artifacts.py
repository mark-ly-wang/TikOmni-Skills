...
WORK_ANALYSIS_ARTIFACT_VERSION = "author_home.work_analysis_artifact@v2"
...
def build_single_work_payload(
    *,
    ...
    payload: Dict[str, Any] = {
        ...
        "create_time_sec": _coerce_unix_sec(work.get("publish_time")),
        "duration_ms": _safe_int(work.get("duration_ms"), default=0),
        "summary": _safe_text(work.get("summary")),
        "insights": ["source=author_home_work_analysis_artifact", "analysis_strategy=single_work_writer"],
        "raw_content": _safe_text(work.get("asr_raw")),
        "asr_clean": _safe_text(work.get("asr_clean")),
        "asr_source": _safe_text(work.get("asr_source")) or "fallback_none",
        "tags": list(work.get("tags") or []) if isinstance(work.get("tags"), list) else [],
        "content_type": _safe_text(work.get("content_type")) or ("video" if platform == "douyin" else "note"),
        "request_id": work.get("request_id"),
        "confidence": "medium" if _safe_text(work.get("asr_clean") or work.get("asr_raw") or work.get("title")) else "low",
        "error_reason": None,
        "extract_trace": [],
    }
    payload.update(_metrics_from_work(work))
    if platform == "xiaohongshu":
        payload["xhs_user_id"] = author_platform_id
    if platform == "douyin":
        payload["douyin_aweme_author_id"] = author_platform_id
    return payload
